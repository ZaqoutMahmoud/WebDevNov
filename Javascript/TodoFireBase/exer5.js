...
text.addEventListener("keyup", (e) => {
  ...
  for (let i of sortedNames) {
    if (
      i.toLowerCase().startsWith(text.value.toLowerCase()) &&
      text.value != ""
    ) {
...
function generateItems(items, atype = "all") {
      ...
      itemsHTML =
        ...
        `<div class="todo-item"> 
          ...
          item.status == "completed" ? "checked" : "nothing"
              ...
              <div  class="binmarkcont ${
                item.status == "completed" ? "checked" : "nothing"
              }"  data-id="${item.id}">
              <img class="binmark  ${
                item.status == "completed" ? "checked" : "nothing"
              }" src="./assets/remove.png" width="45" alt="icon-bin.svg"></div>
          </div>
                  `;
    }
  });
  ...
  item.get().then(function (querySnapshot) {
    document.getElementById("itemsLeft").innerHTML =
      querySnapshot.docs.length + " items left";
